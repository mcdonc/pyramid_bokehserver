...
def prune(request, document, temporary_docid=None, delete=False):
    ...
    if delete:
        for obj in to_delete:
            request.registry.backbone_storage.del_obj(storage_id, obj)
...
class BokehServerTransaction(object):
    ...
    """
    def __init__(self, request, server_userobj, server_docobj, mode,
            ...
            if mode == 'rw':
                if not can_write:
                    raise AuthenticationException(
                        "could not write to %s" % docid)
            elif mode == 'r':
                if not can_read:
                    raise AuthenticationException(
                        "could not read from %s" % docid)
        self.mode = mode
        if self.mode == 'rw':
    ...
    def load(self, gc=False):
        ...
        if self.temporary_docid:
            temporary_json = self.request.registry.backbone_storage.pull(
                self.temporary_docid)
        ...
        if gc and self.mode != 'rw':
            raise AuthenticationException(
                "cannot run garbage collection in read only mode")
        elif gc and self.mode == 'rw':
            prune(self.request, clientdoc, delete=True)
        else:
            prune(self.request, clientdoc)
        init_bokeh(self.request, clientdoc)
        self.clientdoc = clientdoc

    def save(self):
        if self.mode != 'rw':
            raise AuthenticationException("cannot save in read only mode")
        self.changed = self.request.registry.backbone_storage.store_document(
            self.clientdoc,
            temporary_docid=self.temporary_docid
        )
